...
class UtilsSingleton {
	...
	*/
	 ...
	 * @param {number} pAngleInDegrees - The angle (in degrees) at which to move the point.
	 ...
	 */
	calculateNewPositionFromDistanceAndAngle(pPoint, pDistance, pAngleInDegrees) {
		const newPosition = { x: 0, y: 0 };
		const angleInRadians = this.degreesToRadians(pAngleInDegrees);
		newPosition.x = pPoint.x - pDistance * Math.cos(angleInRadians);
		newPosition.y = pPoint.y - pDistance * Math.sin(angleInRadians);
		return newPosition;
	};
	/**
